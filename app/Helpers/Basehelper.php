...
class BaseHelper
{
    ...
    public static function generateIdCard($candidate_id)
    {
        ...
        if ($info->index_card_no == NULL || $info->index_card_no == '') {
            ...
            $records = CandidateEduDetails::where('candidate_id', $candidate_id)->join('exams', 'candidate_edu_details.exam_id', '=', 'exams.id')
                ->where('exams.exam_category', 'x1')
                ->get();
            if (count($records) > 0)
                $category = 'X1';

            $records = CandidateEduDetails::where('candidate_id', $candidate_id)->join('exams', 'candidate_edu_details.exam_id', '=', 'exams.id')
                ->where('exams.exam_category', 'x2')
                ->get();
            if (count($records) > 0)
                $category = 'X2';
            $records = CandidateEduDetails::where('candidate_id', $candidate_id)->join('exams', 'candidate_edu_details.exam_id', '=', 'exams.id')
                ->where('exams.exam_category', 'x3')
                ->get();
            if (count($records) > 0)
                $category = 'X3';
    ...
    public static function moneyFormatCambodia($num)
    {
