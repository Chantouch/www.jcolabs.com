...
@section('content-header')
    View Job |
    <small>{{ $results->emp_job_id }} </small>
@endsection
                ...
                <div class="box-body job-view">
                    ...
                    <div class="row" style="margin-top:10px;">
                        <div class="col-md-12 project-add-info">
                            ...
                            <a href="{{route('employer.postJobs.edit', $results->id) }}" class="btn btn-success btn-sm"><span
                                        class="glyphicon glyphicon-edit"></span> EDIT</a>
                            @if($results->status==1)
                                <a href="{{route('employer.update_job_status_filled_up', Hashids::encode($results->id)) }}"
                            ...
                            @elseif($results->status==0 || $results->status==2)
                                <a href="{{route('employer.update_job_status_active', Hashids::encode($results->id)) }}"
                                   class="btn bg-olive btn-flat btn-sm">
                                    <i class="fa fa-archive"></i> &nbsp;Marked Status as Active</a>
                            @endif
                            @if($results->status!=0)
                                <a href="{{route('employer.update_job_status_disabled', Hashids::encode($results->id)) }}"
                    ...
                    <div class="row" style="margin-top:20px">
                        <div class="col-md-8 project-add-info col-md-offset-2">
                            <i class="fa fa-bullseye"></i> Job Status {!! $results->job_status !!} | <i
                                    class="fa fa-calendar-check-o"></i>Job created at
                            <strong>{{ date('d-m-Y h:i A', strtotime($results->created_at)) }}</strong> | <i
                                    class="fa fa-get-pocket"></i>Employer
                            <strong>{{ $results->employer['organization_name'] }} </strong>
                        </div>
                    </div>
